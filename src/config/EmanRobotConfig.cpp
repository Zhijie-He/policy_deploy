...
{   
    ...
    if (root_["effort_limit"] && root_["effort_limit"].IsSequence() && root_["effort_limit"].size() > 0) {
        ...
        if(effort_vec.size() != num_actions ){
            FRC_INFO("[EmanRobotConfig.Const] Length mismatch between effort_limit and num_actions!");
        ...
        FRC_WARN("[EmanRobotConfig.Const] 'effort_limit' not set or empty, using +inf.");
