...
/*!
 * Go from ZYX rpy to rotation matrix.
 */
    template <typename T>
  ...
  template <typename T>
  Mat3<typename T::Scalar> rpyToRotMat_Jy(const Eigen::MatrixBase<T>& v)
  {
    static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3, "must have 3x1 vector");
    Eigen::AngleAxis<typename T::Scalar> yawAngle(v[2], Vec3<typename T::Scalar>::UnitZ());
    Eigen::AngleAxis<typename T::Scalar> pitchAngle(v[1], Vec3<typename T::Scalar>::UnitY());
    Eigen::AngleAxis<typename T::Scalar> rollAngle(v[0], Vec3<typename T::Scalar>::UnitX());
    Eigen::Quaternion<typename T::Scalar> q = yawAngle * pitchAngle * rollAngle;
    return q.matrix().transpose();
  }

/*!
 * Go from XYZ rpy to rotation matrix.
 */
    template <typename T>
    Mat3<typename T::Scalar> rpyXYZToRotMat(const Eigen::MatrixBase<T>& v) {
      static_assert(T::ColsAtCompileTime == 1 && T::RowsAtCompileTime == 3,
                    "must have 3x1 vector");
...
/*!
 * For Vec4 input, quaternion's convention is WXYZ
